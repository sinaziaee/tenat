...
class GraphChangeNotifier extends ChangeNotifier {
  ...
  double graphHeight = 1000; //default value, change ahead of time
  ...
  void calculateAttraction() {
    ...
    for (Edge edge in graph.edges) {
      ...
      displacement[nodeA] = displacement[nodeA]! - attractionVector;
      displacement[nodeB] = displacement[nodeB]! + attractionVector;
    }
  }

  void moveNodes() {
    for (Node node in graph.nodes) {
      var temp = displacement[node];
      node.position = node.position + temp!;
